...
class FanoronaActionType(Enum):

    MOVE = 1

class FanoronaAction(Action):
    ...
    def __init__(self, action_type, win_by='APPROACH', **kwargs):
        """This is the format that every action must have. Dependending of the action type additional parameters can be asked.
            Example : a move from (0, 1) to (0, 2) is equivalent to FanoronaAction(action_type=FanoronaActionType.MOVE, at=(0, 1), to=(0, 2))

        Args:
            action_type (FanoronaActionType): The type of the performed action.
        """
        assert isinstance(action_type, FanoronaActionType), "Not a good action type format"
        self.action_type = action_type

        if action_type == FanoronaActionType.MOVE:
